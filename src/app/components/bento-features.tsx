          ...
          <div className="relative">
            {[
              ...
              <motion.div
                ...
                className={`absolute w-16 h-16 rounded-full ${avatar.color} flex items-center justify-center text-white font-bold border-4 border-[#1a1f3a] shadow-lg cursor-pointer`}
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className={`text-4xl md:text-5xl font-bold text-white mb-4 ${language === 'ar' ? 'font-cairo-display' : ''}`}>
            ...
            <span className="bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent inline-block pb-1">
          ...
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                {/* Glow Effect */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${feature.gradient} rounded-3xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`}
                />

                {/* Card */}
                <div className="relative h-full bg-gradient-to-br from-[#1a1f3a] to-[#0f1320] rounded-3xl border border-white/10 group-hover:border-white/20 transition-all duration-500 overflow-hidden shadow-xl shadow-black/20">
                  {/* Border Glow */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${feature.borderGradient} opacity-0 group-hover:opacity-20 transition-opacity duration-500 rounded-3xl pointer-events-none`}
                  />

                  <div
                    className={`relative p-8 ${
                      index === 0 ? "grid md:grid-cols-2 gap-8 items-center" : ""
                    }`}
                  >
                    {/* Content */}
                    <div className="space-y-4">
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 flex items-center justify-center shadow-inner">
                        <Icon className="w-7 h-7 text-cyan-400 drop-shadow-md" />
                      </div>
                      <h3 className={`text-2xl font-bold text-white ${language === 'ar' ? 'font-cairo-display' : ''}`}>
                        {feature.title}
                      </h3>
                      <p className={`text-gray-400 leading-relaxed ${language === 'ar' ? 'font-cairo-body' : ''}`}>
                        {feature.description}
                      </p>
                    </div>
